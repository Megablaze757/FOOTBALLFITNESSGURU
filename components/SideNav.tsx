  ...
  return (
    <aside className="sticky top-0 hidden h-screen w-64 shrink-0 flex-col border-r border-white/10 bg-white/[0.02] px-4 py-6 backdrop-blur-xl lg:flex">
      <Link href="/home" className="mb-8 flex items-center gap-2 px-2">
        ...
        {!launched && <span className="rounded-md bg-pitch-400/15 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-accent-400">Beta</span>}
      </Link>
              ...
              <li key={item.href}>
                <Link
                  ...
                  className={`flex items-center gap-3 rounded-2xl px-3 py-2.5 text-sm font-medium transition ${
                    active
                      ? "bg-pitch-400/10 text-accent-400 shadow-[inset_0_0_0_1px_rgba(227,181,63,0.25)]"
      ...
      >
        <div className="text-xs font-semibold uppercase tracking-wider text-accent-400">PocketAthlete Pro</div>
        <p className="mt-1 text-xs text-slate-400">Unlock AI programs, video biomechanics &amp; coaching.</p>
        <span className="mt-2 inline-block text-xs font-bold text-accent-400">Upgrade →</span>
      </Link>
    </aside>
  );
}
