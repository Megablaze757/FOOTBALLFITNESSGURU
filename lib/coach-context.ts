// =============================================================================
// Everything the coach knows about one athlete, in one call.
//
// WHY IT LEFT THE PAGE. It was the body of /ask's loader, which was fine while
// /ask was the only way to reach the coach. It is not: the coach is a floating
// bubble on every screen now, and a bubble that assembled its own briefing
// would be a second answer to "what does the coach know" — the two would drift,
// and the drift would show up as the coach knowing about your rehab plan on one
// screen and not on another.
//
// TWELVE QUERIES, so it is deliberately NOT run on mount. The bubble calls it
// the first time somebody opens the chat and never again in that session; /ask
// calls it through the same cache key, so arriving at the page after using the
...
import type { ChatContext } from "@/lib/coach-chat";
...
/** The cache key both callers share, so the work happens once per athlete. */
export const coachContextKey = (userId: string) => `ask:${userId}`;
...
export async function loadCoachContext(userId: string): Promise<CoachContext> {
  const today = todayLocal();
    ...
    const [
      ...
      supabase.from("programs").select("*").eq("user_id", userId).eq("status", "active")
      ...
      supabase.from("daily_check_ins").select("check_in_date, sleep_quality, fatigue_score").eq("user_id", userId)
      ...
      supabase.from("strength_benchmarks").select("*").eq("user_id", userId)
      ...
      supabase.from("nutrition_logs").select("calories_eaten, macros").eq("user_id", userId).eq("log_date", today).maybeSingle(),
      supabase.from("nutrition_logs").select("calories_eaten, daily_calorie_target, macros").eq("user_id", userId).gte("log_date", since14),
      /**
       ...
       */
      supabase.from("rehab_plans").select("*").eq("user_id", userId).eq("active", true)
        .order("created_at", { ascending: false }).limit(1).maybeSingle(),
    ]);
    ...
    const briefing = buildBriefing({
      ...
      benchmarks: latestMetrics((benches.data ?? []) as StrengthBenchmark[]),
    });
